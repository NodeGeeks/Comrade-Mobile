angular.module('comrade.controllers', [])
    ...
    .controller('MainController', function ($scope, $window, $http, $ionicLoading, $state, $location, SocialAccounts, $localStorage) {
        ...
        if ($scope.$userStorage && $scope.$userStorage.accessToken !== angular.undefined) {
            ...
            $http({
                method: 'POST',
                url: 'http://localhost:1337/users/checkAuthToken',
                data: {id: $scope.$userStorage.id, token: $scope.$userStorage.accessToken}
            }).
        ...
        $scope.remeberMe = function() {
            var isRemembered = $scope.isRemembered;
        };
        ...
        $scope.socialLogin = function (provider) {
            ...
            if (provider == "facebook") {
                options = {
                    scope: 'basic, friends, events, create_event, email, notifications, messages',
                ...
                options = {
                    scope: 'basic, friends, events, email, offline_access',
                    response_type: 'code',
                    oauth_proxy: 'https://auth-server.herokuapp.com/proxy',
                    redirect_uri: 'http://localhost/'
                };
            ...
            hello.login(provider, options, function (auth) {
                ...
                hello(provider).api('/me').success(function (r) {
                    ...
                    $http({
                        method: 'POST',
                        url: baseURL + '/users/loginSocialAccount',
                        data: {provider: auth.network, socialID: r.id, firstName: firstName, lastName: lastName}
                    }).
                        success(function (data, status, headers, config) {
    ...
    .controller('DashboardController', function ($scope, $http, $ionicModal, $location, $ionicActionSheet, $localStorage, Notifications, SocialAccounts, $cordovaCamera) {
        ...
        $http({
            method: 'POST',
            url: 'http://localhost:1337/comrades/pendingComradesRequests',
            data: {accessToken: $scope.$userStorage.accessToken, id: $scope.$userStorage.id}
        }).
            success(function (data, status, headers, config) {
                console.log(data);
            }).
        ...
        $scope.linkSocialAccount = function (provider) {
            ...
            if (provider == "facebook") {
                options = {
                    scope: 'basic, friends, events, create_event, email, notifications',
                    redirect_uri: 'http://localhost/'
                };
            } else if (provider == "twitter") {
                options = {
            ...
            hello.login(provider, options, function (auth) {
                hello(provider).api('/me').success(function (r) {
                    ...
                    $http({
                        method: 'POST',
                        url: baseURL + '/users/linkSocialAccount',
                        data: {
                            provider: auth.network,
                            id: $scope.$userStorage.id,
                            socialID: r.id,
                            token: $scope.$userStorage.accessToken
                        }
                    }).
                        success(function (data, status, headers, config) {
                            $localStorage.user = data[0];
    ...
    .controller('ComradesController', function ($scope, Comrades, SocialAccounts, $localStorage, $http) {
        ...
        $scope.sendComradesRequest = function (val) {
            ...
            $http({
                method: 'POST',
                url: 'http://localhost:1337/comrades/sendComradesRequest',
                data: {value: $scope.val, accessToken: $scope.UserData.accessToken, id: $scope.UserData.id}
            }).
                success(function (data, status, headers, config) {
                    alert(data);
                }).
        ...
        $http({
            method: 'POST',
            url: 'http://localhost:1337/comrades/comrades',
            data: {value: $scope.val, accessToken: $scope.UserData.accessToken, id: $scope.UserData.id}
        }).
            success(function (data, status, headers, config) {
                //console.log(data);
    ...
    .controller('ChatController', function ($scope, $stateParams, sockets, Comrades, UserSession, $animate) {
        ...
        socket.on('message', function (data) {
            console.log("Global message: ", data.msg)
        });
    ...
    .controller('LoadingController', function ($scope, $location, $http, $localStorage) {
        ...
        if ($scope.$storage && $scope.$storage.user.accessToken !== angular.undefined) {
            ...
            $http({
                method: 'POST',
                url: 'http://localhost:1337/users/checkAuthToken',
                data: {id: $scope.$storage.user.id, token: $scope.$storage.user.accessToken}
            }).
                success(function (data, status, headers, config) {
                    if (data === 'false') {
                        $location.path('/main');
